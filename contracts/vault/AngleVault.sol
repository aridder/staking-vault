...
interface ILiquidityGaugeStrat {
    ...
    function deposit_reward_token(address _rewardToken, uint256 _amount)
        external;
    ...
    // solhint-disable-next-line
    function reward_data(address _tokenReward)
        external
        view
...
contract BaseStrategy {
    ...
    modifier onlyGovernanceOrFactory() {
        require(
            msg.sender == governance || msg.sender == vaultGaugeFactory,
            "!governance && !factory"
        );
        _;
    }
    ...
    function toggleVault(address _vault)
        external
        virtual
        onlyGovernanceOrFactory
    {}

    function setGauge(address _token, address _gauge)
        external
        virtual
        onlyGovernanceOrFactory
    {}

    function setMultiGauge(address _gauge, address _multiGauge)
        external
        virtual
        onlyGovernanceOrFactory
    {}
...
contract BaseAccumulator {
    ...
    function _notifyReward(address _tokenReward, uint256 _amount) internal {
        ...
        if (
            ILiquidityGauge(gauge).reward_data(_tokenReward).distributor !=
            address(0)
        ) {
            // Distribute SDT
            ...
            uint256 balanceAfter = IERC20(_tokenReward).balanceOf(
                address(this)
            );
...
interface IGaugeController {
    ...
    //solhint-disable-next-line
    function gauge_relative_weight_write(address addr, uint256 timestamp)
        external
        returns (uint256);

    //solhint-disable-next-line
    function gauge_relative_weight(address addr)
        external
        view
        returns (uint256);

    //solhint-disable-next-line
    function gauge_relative_weight(address addr, uint256 timestamp)
        external
        view
        returns (uint256);
...
abstract contract SdtDistributorEvents {
    ...
    event RewardDistributed(
        address indexed gaugeAddr,
        uint256 sdtDistributed,
        uint256 lastMasterchefPull
    );
...
contract SdtDistributorV2 is
    ReentrancyGuardUpgradeable,
    AccessControlUpgradeable,
    SdtDistributorEvents
{
    using SafeERC20 for IERC20;
    ...
    IERC20 public constant rewardToken =
        IERC20(0x73968b9a57c6E53d41345FD57a6E6ae27d6CDB2F);
    ...
    function initializeMasterchef(uint256 _pid)
        external
        ...
        for (uint256 i; i < lookPastDays; i++) {
            ...
            if (pulls[currentTimestamp] > 0) {
                ...
                if (i == 0) {
                    ...
                    gaugeRelativeWeight = controller
                        .gauge_relative_weight_write(
                            gaugeAddr,
                            currentTimestamp
                        );
                } else {
                    gaugeRelativeWeight = controller.gauge_relative_weight(
                        gaugeAddr,
                        currentTimestamp
                    );
                }

                uint256 sdtDistributed = (sdtBalance * gaugeRelativeWeight) /
                    1e18;
            ...
            if (gaugeType == 1) {
                ...
                if (delegate != address(0)) {
                    ...
                    if (isInterfaceKnown[delegate]) {
                        ISdtMiddlemanGauge(delegate).notifyReward(
                            gaugeAddr,
                            totalDistribute
                        );
                    }
    ...
    function toggleInterfaceKnown(address _delegateGauge)
        external
    ...
    function setTimePeriod(uint256 _timePeriod)
        external
        onlyRole(GOVERNOR_ROLE)
    {
    ...
    function setLookPastDays(uint256 _newLookPastDays)
        external
        onlyRole(GOVERNOR_ROLE)
    {
...
contract AngleStrategy is BaseStrategy {
        ...
        onlyApprovedVault
    {
        ...
        locker.execute(
            _token,
            0,
            abi.encodeWithSignature("approve(address,uint256)", gauge, 0)
        );
        locker.execute(
            _token,
            0,
            abi.encodeWithSignature("approve(address,uint256)", gauge, _amount)
        );
    ...
    function claim(address _token) external override {
        ...
        (bool success, ) = locker.execute(
            gauge,
            0,
            abi.encodeWithSignature("user_checkpoint(address)", address(locker))
        );
        ...
        for (uint8 i = 0; i < 8; i++) {
            ...
            uint256 accumulatorPart = (rewardsBalance * accumulatorFee[gauge]) /
                BASE_FEE;
            uint256 veSDTPart = (rewardsBalance * veSDTFee[gauge]) / BASE_FEE;
            uint256 claimerPart = (rewardsBalance * claimerRewardFee[gauge]) /
                BASE_FEE;
            ...
            uint256 netRewards = rewardsBalance -
                multisigFee -
                accumulatorPart -
                veSDTPart -
                claimerPart;
            IERC20(rewardToken).approve(multiGauges[gauge], netRewards);
            ILiquidityGauge(multiGauges[gauge]).deposit_reward_token(
                rewardToken,
                netRewards
            );
    ...
    function claimerPendingRewards(address _token)
        external
        view
        returns (ClaimerReward[] memory)
    {
        ...
        for (uint8 i = 0; i < 8; i++) {
            ...
            uint256 rewardsBalance = ILiquidityGauge(gauge).claimable_reward(
                address(locker),
                rewardToken
            );
            uint256 pendingAmount = (rewardsBalance * claimerRewardFee[gauge]) /
                BASE_FEE;
            ClaimerReward memory pendingReward = ClaimerReward(
                rewardToken,
                pendingAmount
            );
    ...
    function setRewardsReceiver(address _newRewardsReceiver)
        external
        onlyGovernance
    {
    ...
    function setSdtDistributor(address _newSdtDistributor)
        external
        onlyGovernance
    {
    ...
    function setVaultGaugeFactory(address _newVaultGaugeFactory)
        external
        onlyGovernance
    {
...
contract AngleVault is ERC20Upgradeable {
    ...
    function deposit(
        ...
        require(
            address(liquidityGauge) != address(0),
            "Gauge not yet initialized"
        );
    ...
    function withdraw(uint256 _shares) public {
        uint256 userTotalShares = ILiquidityGaugeStrat(liquidityGauge)
            .balanceOf(msg.sender);
        require(_shares <= userTotalShares, "Not enough staked");
        ILiquidityGaugeStrat(liquidityGauge).withdraw(
            _shares,
            msg.sender,
            true
        );
